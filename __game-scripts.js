...
Input.prevMouseX = 0, Input.prevMouseY = 0, Input.mouseDown = !1, Input.mouseDownPrev = !1, Input.mouseX = 0, Input.mouseY = 0, Input.mousePressed = !1, Input.prototype.postUpdate = function(o) {
...
Game.tempPos = new pc.Vec3, Game.tempPos2 = new pc.Vec3, Game.instance = null, Game.attributes.add("_LEVEL_NUMBER", {
    ...
    type: "entity",
    array: !0
}), Game.attributes.add("uiAdLoading", {
    type: "entity"
...
}, Game.prototype.showStreakText = function(e, t, a, i, n, s = 1) {
    var o = ObjectPool.pop("StreakText", this.canvas2);
    o.setLocalScale(0, 0, 0), this.canvas2.addChild(o), o.element.text = e, o.element.color = i, o.setPosition(0, a, 0), o.script.gameText.animate(2, s), o.enabled = !0
}, Game.prototype.showText = function(e, t, a, i, n, s = 1) {
    var o = ObjectPool.pop("MsgText", this.canvas2);
    return o.setLocalScale(0, 0, 0), this.canvas.addChild(o), o.element.text = e, o.element.color = i, o.setPosition(t, a, 0), o.script.gameText.animate(n, s), o.enabled = !0, o.script.gameText
}, Game.lvlTextShown = !1, Game.prototype.showLvlText = function(e, t, a) {
    Game.lvlTextShown = !0, GameAudio.play("swoosh2");
    var i = ObjectPool.pop("LevelText", this.canvas2);
    i.setLocalScale(1, 1, 1), this.canvas.addChild(i), i.element.text = e, i.setPosition(t, a, -3);
    var n = i.getLocalPosition();
    return i.tween(n).to(new pc.Vec3(n.x, n.y + 2850, n.z), 2.6, pc.CubicOut).loop(!1).yoyo(!1).delay(2).start(), i.enabled = !0, i
}, Game.prototype.initialize = function() {
    for (window.addEventListener("keydown", (e => {
    ...
    for (var i = 0; i < this.finish.children.length; i++)(t = this.finish.children[i]).script && t.script.collBox && t.script.collBox.init();
    ...
    for (var t, a, i = this.levelObjectsE.children.length - 1; i >= 0; i--)
        if ((t = this.levelObjectsE.children[i]).enabled)
            for (var n = t.children.length - 1; n >= 0; n--)(a = t.children[n]).enabled && ("Molot" != a.name && "SpikeMoving" != a.name && "Spike" != a.name || a.getPosition().distance(e) < 8 && a.destroy())
    ...
    this.levelObjectsE.addChild(t), t.setPosition(this.lastPos.x - i.x, -i.y, 0), t.enabled = !0;
    var n = t.findByName("End"),
        s = n.getPosition();
    t.removeChild(a), t.removeChild(n), this.lastPos.copy(s);
    var o, l = t.findByName("StartPoint"),
        r = l.getPosition();
    if (t.removeChild(l), 0 == this.addedLevelsCount) Knife.instance.revive(), Game.KNIFE_NOT_SET_POS_ON_RESTART || (r ? (r.y += 1.35, r.z = 0, r.x -= .55, Knife.instance.entity.setPosition(r)) : Knife.instance.entity.setPosition(0, 3, 0)), Knife.instance.entity.setLocalEulerAngles(0, 0, 125), Knife.instance.stuck(), Game.KNIFE_NOT_SET_POS_ON_RESTART = !1;
    else
        for (var d, h, m = 0; m < t.children.length; m++)
            if ((d = t.children[m]).enabled && "Ground" == d.name) {
                var c = d.getPosition();
                (h = c.y + d.getLocalScale().y / 2) > this.levelUpperPlank && (this.levelUpperPlank = h), Math.abs(c.x - r.x) < .8 && (d.enabled = !1)
    ...
    for (m = 0; m < t.children.length; m++)
        if ((d = t.children[m]).enabled && !d.script)
            for (var p = d.children.length - 1; p >= 0; p--) EntityTools.reparent(d.children[p], t);
    for (m = 0; m < t.children.length; m++)(d = t.children[m]).script && d.script.collBox && d.script.collBox.init(), d.tags.has("deadzone") && (d.render.enabled = !1);
    for (m = 0; m < t.children.length; m++) {
        (d = t.children[m]).script && d.script.physScaler && d.script.physScaler.init();
        for (p = 0; p < d.children.length; p++)(o = d.children[p]).script && o.script.sliceable && o.script.physScaler && o.script.physScaler.init()
    }
    for (m = 0; m < t.children.length; m++)(d = t.children[m]).script && d.script.stackCreator && d.script.stackCreator.init();
    Polygon.initPolygonsOnEntity(t, !0)
}, Game.prototype.kickSliceablesOnPos = function(e, t, a) {
    for (var i, n, s, o = 0; o < this.levelObjectsE.children.length; o++) {
        s = this.levelObjectsE.children[o];
        for (var l = 0; l < s.children.length; l++)(i = s.children[l]).enabled && i.script && i.script.sliceable && (n = i.script.polygon, Math.abs(n.pos.x - e.x) < t && Math.abs(n.pos.y - e.y) < a && i.script.sliceable.kick(e))
    }
    ...
    this.currLevel <= 3 ? (i = !0, a = 1, n = !1) : (this.currLevel >= 8 && Math.random() > .8 && (a = 2), this.currLevel >= 4 && Math.random() > .8 && (n = !0)), e && (a = 1);
    for (var s = 0; s < a; s++) e ? (t = Game.bonusLevelIdsShuffled.pop()) || (Game.bonusLevelIdsShuffled = [...Game.bonusLevelIds], MathUtil.shuffleArray(Game.bonusLevelIdsShuffled), t = Game.bonusLevelIdsShuffled.pop(), Game.debugOutput && console.log("bonus levels shuffled: ", Game.bonusLevelIdsShuffled)) : 0 == s && n ? (t = Game.highLevelIdsShuffled.pop()) || (Game.highLevelIdsShuffled = [...Game.highLevelIds], MathUtil.shuffleArray(Game.highLevelIdsShuffled), t = Game.highLevelIdsShuffled.pop(), Game.debugOutput && console.log("high levels shuffled: ", Game.highLevelIdsShuffled)) : i ? (t = Game.easyLevelIdsShuffled.pop()) || (Game.easyLevelIdsShuffled = [...Game.easyLevelIds], MathUtil.shuffleArray(Game.easyLevelIdsShuffled), t = Game.easyLevelIdsShuffled.pop(), Game.debugOutput && console.log("easy levels shuffled: ", Game.easyLevelIdsShuffled)) : (t = Game.levelIdsShuffled.pop()) || (Game.levelIdsShuffled = [...Game.normalLevelIds], MathUtil.shuffleArray(Game.levelIdsShuffled), t = Game.levelIdsShuffled.pop(), Game.debugOutput && console.log("levels shuffled: ", Game.levelIdsShuffled)), e ? Game.bonusIds.push(t) : Game.levelIds.push(t);
...
}, Game.testAPI = !1, Game.prototype.updateStreak = function(e) {
    ...
    if (this.inputBlockTimer > 0 ? (Game.instance.uiAdLoading.enabled = !0, this.inputBlockTimer -= e, this.inputBlockTimer <= 0 && (this.inputBlockTimer = 0, Input.mouseDis = !1)) : Game.instance.uiAdLoading.enabled = !1, window.scrollTo(0, 10), this.setResolution(), this.shopRewardCooldownCurr > 0 && (this.shopRewardCooldownCurr -= e), Game.state == Game.STATE_PLAYING) {
        ...
        var t = window.innerWidth,
            ...
            i = CameraController.instance;
        i.yaw = pc.math.lerp(-42, -25, pc.math.clamp((t - a) / a, 0, 1)), i.distance = pc.math.lerp(20, 15, pc.math.clamp((t - a) / a, 0, 1))
    }
...
Knife.tempVec = new pc.Vec3, Knife.tempVec2 = new pc.Vec3, Knife.attributes.add("customStickPoint", {
    ...
    if (e.tags.has("ground") && 1 == this.state) {
        if (t.findContact(this.bladeC), Knife.tempVec.copy(Polygon.contactNormal), Knife.tempVec.mulScalar(.5 * Polygon.contactDepth - .05), this.entity.translate(Knife.tempVec), this.stuck(), "kinematic" == e.rigidbody.type && (this.groundStartPos.copy(t.pos), this.groundKnifePosStart.copy(this.entity.getPosition()), this.ground = t), e.tags.has("finish")) {
            ...
            var n = !1;
            Game.instance.bonusMultiplier = 1, Game.instance.bonusOperator && s && (Game.instance.bonusMultiplier = s.count), Game.instance.bonusOperator ? 50 == s.count && Game.instance.bonusReady ? (GameAudio.play("bonushit"), n = !0) : GameAudio.play("xhit") : GameAudio.play("finishhit"), Game.instance.onLevelCompleted(n);
    ...
    if (e.tags.has("sliceable")) {
        ...
        var l = 0;
        a.complexSlice && (l = (this.bladeC.pos.z - t.pos.z) / t.zSize + .5), a.slice(l), this.sliceCd = .1, this.vel.x > 0 && (this.vel.x -= .5, this.vel.x < 0 && (this.vel.x = 0))
    }
    if (e.tags.has("spike")) {
        for (var h = 0; h < 8; h++) EffectDrop.create(this.entity.getPosition(), pc.math.random(.3, .5), new pc.Vec3(pc.math.random(-7, 7), pc.math.random(4, 7), pc.math.random(-4, 4)), 2, Game.instance.whiteColor);
        this.kill("spikes")
    }
    ...
    else if (Game.instance.controlsEnabled && !Input.mouseDis) {
        var i = !1;
        if (UiMainMenu.instance && UiMainMenu.instance.isFirstKnifeHintShown() && (i = !0), !i && (Input.mousePressed && e > .1 && Game.state == Game.STATE_PLAYING || Input.mousePressed && e > .1 && Input.mouseX > 0 && Game.state == Game.STATE_INTRO || this.app.keyboard.wasPressed(pc.KEY_SPACE) && (Game.state == Game.STATE_INTRO || Game.state == Game.STATE_PLAYING))) {
            this.jumpCd = .15, Game.instance.onJump(), this.rotVel -= Knife.ROT_VEL_MAX;
            var s = new pc.Vec3(4.2, 11.7, 0);
            this.bodyC.pos.y > Game.instance.levelUpperPlank && (s.y = 1), this.vel.copy(s), this.vel.x < s.x && (this.vel.x = s.x), this.vel.y < s.y && (this.vel.y = s.y), this.dampCd = .45, 0 == this.state ? this.stuckCd = .4 : this.stuckCd = .15, this.bounceCd = 0, this.unstuck(), Game.instance.onKnifeOutGround()
        }
    }
    ...
    for (var n = .3333333333333333 * t, o = 0; o < 3; o++) {
        if (1 == this.state) {
            Knife.tempVec2.set(0, -23 * n, 0), this.vel.add(Knife.tempVec2);
            var a = this.entity.getEulerAngles(),
                l = Math.abs(MathUtil.angleDifference(a.z, 180)),
                h = 1 - l / 40;
            this.rotVel < -Knife.ROT_VEL_MAX && (this.rotVel = -Knife.ROT_VEL_MAX), this.vel.y > 28 && (this.vel.y = 28), this.vel.y < -25 && (this.vel.y = -25), this.vel.x > 4.2 && (this.vel.x = 4.2), h < 0 ? this.rotVel = pc.math.lerp(this.rotVel, -Knife.ROT_VEL_MAX, 10 * n) : this.dampCd <= 0 && (this.rotVel = pc.math.lerp(this.rotVel, -85, 17 * n)), this.dampCd <= 0 && this.sliceCd > 0 && a.z + 180 > 300 && l < 60 && (a.z += MathUtil.angleDifference(a.z, 170) * n * 11, this.entity.setEulerAngles(a)), this.entity.rotate(0, 0, this.rotVel * n)
        }
        Knife.tempVec.copy(this.vel), Knife.tempVec.mulScalar(n), Knife.tempVec.add(this.entity.getPosition()), Knife.tempVec.y > Game.instance.levelUpperPlank && (Knife.tempVec.y = pc.math.lerp(Knife.tempVec.y, Game.instance.levelUpperPlank, .25)), this.entity.setPosition(Knife.tempVec), this.bodyC.updatePoints(), this.bladeC.updatePoints(), this.bounceCd <= 0 && this.bodyC.checkAllCollisions(0), 0 != this.state && this.stuckCd <= 0 && this.bladeC.checkAllCollisions(0), this.bodyC.checkAllCollisions(2), this.bladeC.checkAllCollisions(2)
    }
...
FinishController.attributes.add("blocks", {
    ...
    var r, l, i = FinishController.blockData[0].blockType,
...
FinishController.prototype.generateRandomBlockData = function() {
    ...
    t = Math.random() > .6 ? (OperatorType.SUBTRACT, OperatorType.DIVIDE) : this.getRandomPositiveOperator(), this.blockTypes.push(this.createBlock(e)), this.blockTypes.push(this.createBlock(t)), this.blockTypes.sort(((t, e) => t.weight - e.weight))
    ...
    for (var t, e, o = [2, 3, 5, 10, 25, 50], r = 0; r < this.blocks.length; r++)(t = this.textEntity.clone()).element.text = o[r].toString() + " x", this.entity.addChild(t), e = this.blocks[r].getLocalScale().y, FinishController.blockData.push({
        text: t,
        count: o[r],
    ...
    for (var e = 0; e < FinishController.blockData.length; e++) {
        var o = FinishController.blockData[e],
            r = o.entity,
            l = r.getPosition(),
            i = (r.getLocalScale(), l.y - .5 * o.sizeY),
            a = l.y + .5 * o.sizeY;
...
Molot.attributes.add("delay", {
    ...
    if (this.delay > 0) this.delay -= e;
        ...
        t = i < .5 ? 2 * i * i : (4 - 2 * i) * i - 1, this.angle = 180 * t - 90, this.entity.setEulerAngles(this.angle, 0, 0)
    }
};
...
UiCompleted.attributes.add("textColors", {
    ...
    for (var i = 0; i < e; i++) {
        var s = this.serpentinePrefab.clone();
        s.enabled = !0;
        var o = pc.math.random(-n, n),
            a = pc.math.random(t.y, t.y + n),
            r = t.z,
            l = pc.math.random(-.3, .3),
            c = pc.math.random(-.6, .6);
        l = pc.math.random(-3, 3), c = pc.math.random(-3, 3), s.script.uiSerpantine.init(), s.script.uiSerpantine.velocity.x = l, s.script.uiSerpantine.velocity.y = c, this.entity.addChild(s), s.setLocalPosition(o, a, r), this.serps.push(s)
    }
...
ShopButton.attributes.add("newS", {
    ...
    type: "entity"
}), ShopButton.instance = null, ShopButton.prototype.initialize = function() {
    ShopButton.instance = this, this.time = 1.5, this.newS.enabled = !1, this.newS2.enabled = !1, this.onEnable(), this.on("enable", this.onEnable, this)
...
}, ShopButton.prototype.update = function(t) {
    if (this.time += t, this.time > 2) {
        this.time = 0;
...
}, ShopButton.prototype.isNewKnifeAvailable = function() {
    return this.newS && this.newS.enabled
};
...
UiMainMenu.instance = null, UiMainMenu.prototype.initialize = function() {
    UiMainMenu.instance = this, this.caption = this.entity.findByName("Caption"), this.levelText = this.entity.findByName("CurrLevel"), this.firstKnifeHint = this.entity.findByName("FirstKnifeHint"), this.initialHeight = null;
    var i = this.entity.screen;
    this.initialHeight = i.referenceResolution.y, this.onEnable(), this.on("enable", this.onEnable, this)
}, UiMainMenu.prototype.isFirstKnifeHintShown = function() {
    return this.firstKnifeHint && this.firstKnifeHint.enabled
}, UiMainMenu.hideCap = !1, UiMainMenu.prototype.onEnable = function() {
    var i = this.entity.screen;
    window.innerHeight > window.innerWidth ? (i.referenceResolution.y = this.initialHeight + 170, i.resolution = new pc.Vec2(i.referenceResolution.x, i.referenceResolution.y)) : (i.referenceResolution.y = this.initialHeight, i.resolution = new pc.Vec2(i.referenceResolution.x, i.referenceResolution.y)), 1 == Game.instance.currLevel && (UiMainMenu.hideCap = !1), UiMainMenu.hideCap ? (this.caption.enabled = !1, Game.instance.setupPlayingCamera()) : this.caption.enabled = !0, this.levelText.enabled = this.caption.enabled
}, UiMainMenu.prototype.update = function(i) {
    var e = !1;
    ShopButton.instance && this.firstKnifeHint && 0 == Game.instance.firstKnifeTapped && ShopButton.instance.isNewKnifeAvailable() && (e = !0), this.firstKnifeHint.enabled != e && (this.firstKnifeHint.enabled = e)
};
...
UiGameplay.prototype.initialize = function() {
    ...
    var e = this.entity.screen;
    window.innerHeight > window.innerWidth ? (e.referenceResolution.y = this.initialHeight + 170, e.resolution = new pc.Vec2(e.referenceResolution.x, e.referenceResolution.y)) : (e.referenceResolution.y = this.initialHeight, e.resolution = new pc.Vec2(e.referenceResolution.x, e.referenceResolution.y))
...
class CrazyGamesSDKHandler {
    constructor() {
        this.sdkLoaded = !1, this.onSDKReady = null, this.muteSoundState = !1, this.muteMusicState = !1
    }
    async initSDK() {
        try {
            await window.CrazyGames.SDK.init(), this.sdkLoaded = !0, console.log("CrazyGames SDK initialized successfully."), this.onSDKReady && this.onSDKReady()
        } catch (e) {
